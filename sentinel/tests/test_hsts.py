...
class TestHSTSAnalyzer:
    ...
    def test_excellent_without_preload(self):
        ...
        quality, issues, recs, types = HSTSAnalyzer.analyze("max-age=31536000; includeSubDomains")
    ...
    def test_good_without_subdomains(self):
        ...
        quality, issues, recs, types = HSTSAnalyzer.analyze("max-age=31536000")
    ...
    def test_short_max_age_below_minimum(self):
        ...
        quality, issues, recs, types = HSTSAnalyzer.analyze("max-age=86400")  # 1 day
    ...
    def test_missing_max_age(self):
        ...
        quality, issues, recs, types = HSTSAnalyzer.analyze("includeSubDomains")

        assert quality == HeaderQuality.DANGEROUS
        assert any("missing" in issue.lower() and "max-age" in issue.lower() for issue in issues)
        assert HSTSAnalyzer.ISSUE_TYPE_MISSING_MAXAGE in types

    def test_max_age_zero(self):
        """Test max-age=0 which disables HSTS."""
        quality, issues, recs, types = HSTSAnalyzer.analyze("max-age=0")
    ...
    def test_invalid_max_age(self):
        ...
        quality, issues, recs, types = HSTSAnalyzer.analyze("max-age=invalid")
    ...
    def test_preload_without_subdomains(self):
        ...
        quality, issues, recs, types = HSTSAnalyzer.analyze("max-age=31536000; preload")

        assert any(
            "preload" in issue.lower() and "includesubdomains" in issue.lower() for issue in issues
        )

    def test_case_insensitive_directives(self):
        """Test that directives are case-insensitive."""
        quality1, _, _, _ = HSTSAnalyzer.analyze("max-age=31536000; includeSubDomains")
        quality2, _, _, _ = HSTSAnalyzer.analyze("max-age=31536000; IncludeSubDomains")
        quality3, _, _, _ = HSTSAnalyzer.analyze("MAX-AGE=31536000; INCLUDESUBDOMAINS")

        assert quality1 == quality2 == quality3 == HeaderQuality.EXCELLENT

    def test_whitespace_handling(self):
        """Test handling of extra whitespace."""
        quality, issues, _, _ = HSTSAnalyzer.analyze("  max-age=31536000  ;  includeSubDomains  ")
