...
class TestReferrerPolicyAnalyzer:
    ...
    def test_no_referrer_when_downgrade_good(self):
        ...
        quality, issues, recs, types = ReferrerPolicyAnalyzer.analyze("no-referrer-when-downgrade")

        assert quality == HeaderQuality.GOOD
        assert any("default" in issue.lower() or "stricter" in issue.lower() for issue in issues)
    ...
    def test_origin_when_cross_origin_weak(self):
        ...
        quality, issues, recs, types = ReferrerPolicyAnalyzer.analyze("origin-when-cross-origin")
    ...
    def test_unsafe_url_weak(self):
        ...
        assert quality == HeaderQuality.WEAK
        assert any("unsafe" in issue.lower() or "full url" in issue.lower() for issue in issues)
        assert ReferrerPolicyAnalyzer.ISSUE_TYPE_LEAKY in types
    ...
    def test_invalid_policy_weak(self):
        ...
        assert quality == HeaderQuality.WEAK
        assert any("unknown" in issue.lower() or "invalid" in issue.lower() for issue in issues)
    ...
    def test_multiple_policies_last_wins(self):
        ...
        quality1, _, _, _ = ReferrerPolicyAnalyzer.analyze("unsafe-url, no-referrer")
        quality2, _, _, _ = ReferrerPolicyAnalyzer.analyze("no-referrer, unsafe-url")
...
class TestReferrerPolicyRecommendations:
    ...
    def test_recommendations_specific_to_issue(self):
        ...
        _, _, recs_default, _ = ReferrerPolicyAnalyzer.analyze("no-referrer-when-downgrade")
    ...
    def test_no_recommendations_for_excellent(self):
        ...
        for policy in excellent_policies:
            ...
            assert len(recs) == 0 or all("consider" in rec.lower() for rec in recs)
...
class TestReferrerPolicyEdgeCases:
    ...
    def test_multiple_commas(self):
        ...
        quality, issues, _, _ = ReferrerPolicyAnalyzer.analyze("origin,, strict-origin")
    ...
    def test_mixed_valid_invalid_policies(self):
        ...
        quality, issues, _, _ = ReferrerPolicyAnalyzer.analyze("invalid, no-referrer")
