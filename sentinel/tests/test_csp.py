...
class TestCSPParser:
    ...
    def test_parse_whitespace_handling(self):
        ...
        directives = CSPAnalyzer.parse_directives("  default-src  'self'  ;  script-src  'self'  ")
...
class TestCSPAnalysis:
    ...
    def test_missing_default_src(self):
        ...
        quality, issues, recs, types = CSPAnalyzer.analyze("script-src 'self'")
    ...
    def test_wildcard_sources_detected(self):
        ...
        quality, issues, recs, types = CSPAnalyzer.analyze("default-src 'self'; script-src *")
    ...
    def test_http_sources_detected(self):
        ...
        assert quality in (HeaderQuality.WEAK, HeaderQuality.DANGEROUS)
        assert any("http:" in issue.lower() or "insecure" in issue.lower() for issue in issues)
    ...
    def test_report_uri_not_flagged(self):
        ...
        quality, issues, recs, types = CSPAnalyzer.analyze("default-src 'self'; report-uri *")

        # report-uri with * should not trigger wildcard warning
        assert CSPAnalyzer.ISSUE_TYPE_WILDCARD not in types or not any(
            "report-uri" in issue.lower() for issue in issues
        )
    ...
    def test_good_quality_minor_issues(self):
        ...
        quality, issues, recs, types = CSPAnalyzer.analyze("script-src 'self'; style-src 'self'")
    ...
    def test_single_wildcard_weak(self):
        ...
        quality, issues, recs, types = CSPAnalyzer.analyze("default-src 'self'; img-src *")
...
class TestCSPEdgeCases:
    ...
    def test_duplicate_directives_first_wins(self):
        ...
        directives = CSPAnalyzer.parse_directives("default-src 'self'; default-src 'none'")
    ...
    def test_very_long_csp(self):
        ...
        long_csp = "default-src 'self'; " + "; ".join([f"script-src-{i} 'self'" for i in range(50)])
