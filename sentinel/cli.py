...
app = typer.Typer(help="Analyze HTTP security headers with precision.", add_completion=False)
...
def analyze(
    ...
    analyzer = SecurityHeadersAnalyzer(
        timeout=timeout, follow_redirects=not no_redirect, max_redirects=max_redirects
    )
...
def main(
    ctx: typer.Context,
    debug: bool | None = typer.Option(None, "--debug", help="Enable debug logging", hidden=True),
) -> None:
    """
