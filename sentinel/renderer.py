...
def render_report(report: SecurityReport, verbose: bool = False) -> None:
    ...
    console.print(Panel(panel_content, title="Sentinel Analysis Report", border_style="cyan"))
    console.print()
    ...
    for analysis in report.analyses:
        ...
        table.add_row(analysis.name, status, quality_display, score_display, details)
    ...
    critical_issues = [
        ...
        if (not a.present and a.expected_severity in (Severity.CRITICAL, Severity.HIGH))
        or (
            a.quality in (HeaderQuality.DANGEROUS, HeaderQuality.WEAK)
            and a.expected_severity in (Severity.CRITICAL, Severity.HIGH)
        )
    ]
    ...
    if verbose and len(report.redirect_chain) > 1:
        redirect_table = Table(
            title="Redirect Chain", box=box.SIMPLE, show_header=True, header_style="bold"
        )
